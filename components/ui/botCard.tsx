...
export const BotCard = ({ message }: { message: Message }) => {
  ...
  return <div className='bg-gray-200/90 rounded-md p-4 border border-gray-300 overflow-x-auto'>
    ...
    {!!message?.parts?.length && message.parts.some(part => part.type === 'tool-invocation' && part.toolInvocation.state !== 'result' && part.toolInvocation.toolName === 'searchDocs') && <div className='flex items-center space-x-2'>
      <span className="italic font-light font-afacad text-lg">Let me get back to you with the answer while I drink my coffee and read the documentation of Ableton Live 12...</span><PiCoffeeFill className='flex-shrink-0' color='black' />
    </div>}
